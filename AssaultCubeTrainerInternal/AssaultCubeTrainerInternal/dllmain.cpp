...
bool bExit = false, bHealth = false, bAmmo = false, bRecoil = false, bRateOfFire = false, bIncrementAmmo = false, bFlyHack = false;

//Get Module Base //Calling it with NULL also gives you the address of the .exe module
...
//define function
typedef BOOL(__stdcall* twglSwapBuffers)(HDC hDc); //t denotes template of the function
//define function pointer
twglSwapBuffers wglSwapBuffersGateway; 
twglSwapBuffers owglSwapBuffers; //o denotes pointer to the original function
...
BOOL __stdcall hkwglSwapBuffers(HDC hDc) //hk denotes our hook function
{
	...
	if (bUpdateDisplay || clock() - timeSinceLastUpdate > 3000) //update every 3000ms or on keypress
	{
		...
		if (bHealth) { std::cout << dye::green(sHealth); }
		else { std::cout << dye::red(sHealth); }
		std::cout << " <-" << std::endl << std::endl;
		std::cout << "[NUMPAD2] Ammo Lock                     -> ";
		if (bAmmo) { std::cout << dye::green(sAmmo); }
		else { std::cout << dye::red(sAmmo); }
		std::cout << " <-" << std::endl << std::endl;
		...
		if (bRateOfFire) { std::cout << dye::green(sRateOfFire); }
		else { std::cout << dye::red(sRateOfFire); }
		std::cout << " <-" << std::endl << std::endl;
	...
	if (GetAsyncKeyState(VK_NUMPAD5) & 1)
	{
		...
		if (bIncrementAmmo)
		{
			...
			HookBasic((void*)hookAddress, ourFunct, hookLength);
			sIncrementAmmo = "ON ";
		}
		else
		{
	...
	//always draw after you do everything else
...
DWORD WINAPI HackThread(HMODULE hModule)
{
	...
	while (true)
	{
		if (bExit)
		{
			...
			fclose(f);
			FreeConsole();
			FreeLibraryAndExitThread(hModule, 0); //not necessary when using swapbuffers hook	
		}
	}
	//Cleanup&eject - this will crash our stdcall if we cout after calling hackthread
